...
class BitBuffer:
    ...
    def read_bit(self):
        ...
        self.increment_bit_ptr()
        return bit

    def read_bits(self, number_of_bits):
        value = 0
        for i in range(number_of_bits):
            value += self.read_bit() << (number_of_bits - 1 - i)
        return value
...
def compress(data, offset_size, length_size):
    ...
    while ptr < len(data):
        offset, length = find_previous_occurrence(data, ptr, max_offset, max_length)
        if offset > 0:
            buffer.write_bit(0)
...
def find_previous_occurrence(data, ptr, max_offset, max_length):
    longest_found = 0
    found_offset = 0
    for i in range(1, min(max_offset, len(data))):
        length = 0
        while (len(data) > ptr + length) and \
                (length + 1 < max_length) and \
                (ptr + length - i >= 0) and \
                (data[ptr + length] == data[ptr + length - i]):
            length += 1
